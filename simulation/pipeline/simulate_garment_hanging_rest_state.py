...
def simulate_garment_hanging_rest_state(sample_config: dict, sample_data: dict,
    ...
    This can then be used with
        simulation.blender_util.physics.set_sim_output_as_default_mesh_shape()
    to set the output of this simulation as the new mesh default state. We can then run additional
    physics simulations (like movement with a "gripper") on this new resting state since Blender
    doesn't support sequential physics simulations (to my knowledge).
    """
    smpl_sim_args = dict()
